...
const OsintIntelligencePanel: React.FC<Props> = () => {
  ...
  const handleInvestigate = async () => {
    ...
    const result = await performOsintInvestigationAI(target);
    setData(result);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-slide-up">
           ...
           <div className="lg:col-span-1 bg-white dark:bg-slate-900 p-8 rounded-[40px] border dark:border-white/5 shadow-sm">
              <div className="flex justify-between items-center mb-8">
                 <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest">SECURITY POSTURE</h4>
              ...
              <div className="space-y-6">
                 <div className="flex justify-between items-center">
                    <span className="text-xs font-bold text-slate-500">DATA BREACH</span>
                    <span className={`text-xs font-black ${data.dataBreachAlert ? 'text-red-500' : 'text-green-500'}`}>
                       {data.dataBreachAlert ? 'DETECTED!' : 'CLEAN'}
                    </span>
                 </div>
                 <div className="p-5 bg-slate-50 dark:bg-white/5 rounded-2xl border border-slate-100 dark:border-white/5">
                    <div className="text-[9px] font-black text-slate-400 uppercase mb-3">EXTRACTED DNS LOG</div>
           ...
           <div className="lg:col-span-2 bg-[#0b0e14] text-white p-10 rounded-[40px] shadow-2xl relative overflow-hidden flex flex-col border border-white/5">
              <div className="relative z-10 flex-1">
                 <h4 className="text-[10px] font-black text-indigo-400 uppercase tracking-widest mb-6">FORENSIC DEDUCTION</h4>
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-8 border-t border-white/10">
                    <div>
                       <h5 className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-4">ASSOCIATED ENTITIES</h5>
