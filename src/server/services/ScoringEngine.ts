...
export const ScoringEngine = {
  phoneticPatternScore(first: string, second: string): number {
    let score = 0;
  ...
  async wordSimilarity(w1: string, w2: string, cache: Map<string, number>): Promise<number> {
  ...
  async nameQualityScore(
    ...
    const lengthScore = (length >= 7 && length <= 10) ? 15 : 5;
    const negativePenalty = (isNegative(first) || isNegative(second)) ? -20 : 0;
    const score = semantic + phonetic + lengthScore + negativePenalty;
    return Math.max(0, score);
  },

  isValidName(first: string, second: string, isNegative: (word: string) => boolean): boolean {
    ...
    return !(isNegative(first) || isNegative(second));
  }
};
